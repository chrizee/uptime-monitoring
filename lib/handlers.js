...
handlers._tokens = {
    ...
    post: (data, callback) => {
        ...
        if(phone && password && phone.length === 11) {
            _data.read('users', phone, (err, data) => {
                _performance.mark("read user from storage");
                _performance.measure("Final measurement", "entered function", "read user from storage");
                //log out measurement
                const measurements = _performance.getEntriesByType("measure");
                measurements.forEach(measurement => {
                    console.log('\x1b[33m%s\x1b[0m', measurement);                    
                });
                if(!err && data) {
                    const hash = helpers.hash(password);
                    ...
                    _data.read('users', phone, (err, userData) => {
                        if(!err && userData) {
                            ...
                            if(userChecks.length < config.maxChecks) {
                                //verify the given url has dns entries
